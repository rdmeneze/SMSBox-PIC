...
bool CIniFile::ReadFile()
{
  ...
  while( getline( f, line)) {
    ...
    // without change.
    if ( line[(unsigned)line.length() - 1] == '\r')
...
/*
CStdString CIniFile::GetValue( unsigned const keyID, unsigned const valueID, CStdString const defValue) const
{
  if ( keyID < keys.size() && valueID < keys[keyID].names.size())
...
*/
/*
CStdString CIniFile::GetValue( CStdString const keyname, CStdString const valuename, CStdString const defValue) const
{
  long keyID = FindKey( keyname);
  if ( keyID == noID)
    return defValue;
...
*/
/*
int CIniFile::GetValueI(CStdString const keyname, CStdString const valuename, int const defValue) const
{
  char svalue[MAX_VALUEDATA];
...
*/
/*
double CIniFile::GetValueF(CStdString const keyname, CStdString const valuename, double const defValue) const
{
  char svalue[MAX_VALUEDATA];
...
unsigned CIniFile::GetValueV( CStdString const keyname, CStdString const valuename, char *format,
...
void CIniFile::GetValue(const char *sValue, const char *sDefaultValue, unsigned short &Value){
  ...
  long keyID = FindKey( sKeyName.c_str() );
  if ( keyID == noID){
    Value = atoi( sDefaultValue );
    return;
  }

  long valueID = FindValue( unsigned(keyID), sValue);
  if ( valueID == noID){
    Value = atoi( sDefaultValue );
    return;
  }
...
void CIniFile::GetValue(const char *sValue, const char *sDefaultValue, bool &Value){
  ...
  GetValue( sValue, sDefaultValue, usValue );
...
void CIniFile::GetValue(const char *sValue, const char *sDefaultValue){
}
