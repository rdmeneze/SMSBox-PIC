...
class CIniFile  : public ConfigInterface
{
private:
...
public:
  ...
  // Overloaded to return CStdString, int, and double.
  // Returns defValue if key/value not found.
  //CStdString GetValue( unsigned const keyID, unsigned const valueID, CStdString const defValue = "") const;
//  CStdString GetValue(CStdString const keyname, CStdString const valuename, CStdString const defValue = "") const; 
  //int    GetValueI(CStdString const keyname, CStdString const valuename, int const defValue = 0) const;
  //bool   GetValueB(CStdString const keyname, CStdString const valuename, bool const defValue = false) const {
  //  return GetValueI( keyname, valuename, int( defValue )) == 0 ? false: true;
  //}
  //double   GetValueF(CStdString const keyname, CStdString const valuename, double const defValue = 0.0) const;
  // This is a variable length formatted GetValue routine. All these voids
  // are required because there is no vsscanf() like there is a vsprintf().
  // Only a maximum of 8 variable can be read.
/*
  unsigned GetValueV( CStdString const keyname, CStdString const valuename, char *format,
...
*/
  ...
  // new implementations for compatibility with the config class
