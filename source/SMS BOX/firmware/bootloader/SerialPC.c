 ...
 *  -- 02/03/2011 by rdias : removing of some parts of the code for optimization to work in BootLoader
 *                              Removed: Log, 
 */
...
void ProcessaCMD_VERSION( BYTE * buffer )
{
  ...
  for ( bCounter = 0; bCounter < GET_VEC_NUM_MEMBERS(dwTmp); bCounter++ )
  {
    for ( i = sizeof( DWORD ) ; i ; i-- )
    {
      bTmp = CMD_FIELD + (bCounter << 2) + (sizeof(DWORD) - (i - 1));
      bufOut[ bTmp ] = ((BYTE*)dwTmp[bCounter])[(i - 1)];
    } 
  }
...
void ProcessaCMD_BOOTLOADER_UPLOAD_FW( BYTE * buffer )
{
  ...
  {
    bufOut[CMD_FIELD+1] = (UpLoad2Flash( (BYTE*)&buffer[CMD_FIELD+1], bLen ) == ERROR_SUCCESS) ? TRUE : FALSE;
  }
