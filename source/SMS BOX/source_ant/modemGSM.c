...
BYTE ModemGSM_ActiveSIMCard;                        //! SIM card que está ativo atualmente.
...
BYTE ModemGSM_ChangeCard( BYTE ID ){
  ...
  if ( ID < NCARD ){
    ModemGSM_ActiveSIMCard = ID + CARD_OFFSET;
  }
...
#elif (( BOARD_VERSION == 0x01010000 ) | ( BOARD_VERSION == 0x01020000 ))
//  SIM_SEL_EN = 1;
  
  if ( ModemGSM_ActiveSIMCard  != -1 ){
    SIM_SEL &= ~SIM_SEL_MASK;
    SIM_SEL |= (ModemGSM_ActiveSIMCard & SIM_SEL_MASK);
    SIM_SEL_EN = 0;
    return TRUE;
  }
  return FALSE;
